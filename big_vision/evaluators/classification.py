...
def get_eval_fn(predict_fn, loss_name):
  ...
  def _eval_fn(train_state, batch, labels, mask):
    ...
    return ncorrect, loss, nseen, out["attn_weight_avg"], out["attn_mag"], out["before_mlp"]
  return _eval_fn
...
class Evaluator:
  ...
  def run(self, train_state):
    ...
    attn_weight_avg, attn_mag, before_mlp, = jnp.empty((0, 12, 281)), jnp.empty((0, 12, 281)), jnp.empty((0, 12, 281))
    for _, batch in zip(range(self.steps), self.get_data_iter()):
      labels, mask = batch.pop(self.label_key), batch.pop('_mask')
      batch_ncorrect, batch_losses, batch_nseen, batch_attn_weight_avg, batch_attn_mag, batch_before_mlp = \
          multihost_utils.process_allgather(self.eval_fn(train_state, batch, labels, mask))
      ncorrect += batch_ncorrect
      loss += batch_losses
      nseen += batch_nseen

      attn_weight_avg = jnp.concatenate((attn_weight_avg, batch_attn_weight_avg), axis=0)
      attn_mag = jnp.concatenate((attn_mag, batch_attn_mag), axis=0)
    ...
    yield ('attn_weight_avg', attn_weight_avg)
    yield ('attn_mag', attn_mag)
